...
if selection[0]:
    ...
    print(f"Zero_extension_CA残差: {np.std(Z_residual_ca_0)*1e9:.2f}")
    
    
    
#%%
Ext_rx, Ext_ry = 0.0, 0.0
# 1. Zero extension
X_ext_no, Y_ext_no, Z_0_no, ca_range = Surface_Extension(
    X, Y, Z, brf_params, Z_avg, 'zero', False, ext_r_x = Ext_rx, ext_r_y = Ext_ry)

# _, _, Z_0, _ = Surface_Extension(
#     X, Y, Z, brf_params, Z_avg, '8nn', False, ext_r_x = Ext_rx, ext_r_y = Ext_ry)



# X_ext started from X[0,0], keep in normal order(small to large)
Z_ext_no = np.empty(X_ext_no.shape) * np.nan  # mark the Z_ext_no to NaN
Z_ext_no[ca_range['y_s']:ca_range['y_e'], ca_range['x_s']:ca_range['x_e']] = Z  # fill points
    


# 分别计算 X 和 Y 方向上的扩展区域大小
r_x = max(X_ext_no.shape[1] - Z.shape[1], 0) // 2
r_y = max(Y_ext_no.shape[0] - Z.shape[0], 0) // 2
u, v = np.meshgrid(np.arange(-r_x, r_x+1), np.arange(-r_y, r_y+1))
...
if selection[0]:
    B, X_B, Y_B, _, _, T_0_no, _, X_P, Y_P, _, _, _, _, _, _, X_ca, Y_ca, _, _, _ = \
        DwellTime2D_FFT_Full_Test(
            X_ext_no, Y_ext_no, Z_0_no, 0, brf_params.copy(), brf_mode, X_brf, Y_brf, Z_avg, ca_range,
            pixel_m, tmin, tmax, options, ratio, False, ext_r_x = Ext_rx, ext_r_y = Ext_ry
        )

    T_0_no[id_ext_no] = 0

    # Z_removal_dw_0_no = scipy.signal.fftconvolve(T_0_no, B,mode='same')
    if 'T_min' in locals():
        T_0_no = T_0_no - np.min(T_0_no) + T_min
    else:
        T_0_no = T_0_no - np.min(T_0_no)

    Z_removal_dw_0_no = conv_fft2(T_0_no, B)
    Z_0_no = remove_surface1(X_ext_no, Y_ext_no, Z_0_no)
    Z_0_no = Z_0_no - np.nanmin(Z_0_no)
    Z_residual_dw = Z_0_no - np.nanmin(Z_0_no) - Z_removal_dw_0_no
    Z_residual_ca_0_no = Z_residual_dw[ca_range['y_s']:ca_range['y_e'], ca_range['x_s']:ca_range['x_e']]
    Z_residual_ca_0_no = remove_surface1(X_ca, Y_ca, Z_residual_ca_0_no)
    print(f"No_ext_noension_CA残差: {np.std(Z_residual_ca_0_no)*1e9:.2f}")
...
fig = plt.figure("One-step surface extension dwell time results",
                 figsize=(4.7*5, 2.7*np.count_nonzero(selection)+1), dpi=100)
# Original surface
...
ax0.set_title(
    f"Original Surface: \nPV = {np.round((np.max(Z) - np.min(Z)) * 1e9, 2)} nm, RMS = {np.round(rms_Z, 2)} nm")
...
ax0.set_title(
    f"BRF: Peakrate = \n{np.round(brf_params['A']*1e9, 3)} nm/s, Sigma = {np.round(brf_params['sigma_xy'][0] * 1e3, 4)} nm")
ax0.invert_yaxis()
...
if selection[0]:
    ...
    c2.set_label('[s]')
    ax2.set_title(
        f"Dwell time =  {np.round((np.sum(T_0)), 2)} s")
...
if selection[1]:
    ax1 = fig.add_subplot(grid[plot_idx, 0])
    mesh1 = ax1.pcolormesh(X_ext_no * 1e3, Y_ext_no * 1e3, Z_0_no * 1e9, cmap='viridis')
    ax1.set_aspect('equal')
    c1 = plt.colorbar(mesh1, ax=ax1, pad=0.05, fraction=fraction_bar, shrink=1, aspect=np.shape(
        Z_ext_no)[0]/np.shape(Z_ext_no)[1]*((1-fraction_bar)/fraction_bar))
    c1.set_label('[nm]')
    ax1.set_title(
        f"No extension: \nPV = {np.round((np.max(Z_0_no) - np.min(Z_0_no)) * 1e9, 2)} nm, RMS = {np.round(np.std(Z_0_no) * 1e9, 2)} nm")
    # fig.gca().invert_yaxis()
    ...
    # ...
    ax2 = fig.add_subplot(grid[plot_idx, 1])
    mesh2 = ax2.pcolormesh(X_ext_no * 1e3, Y_ext_no * 1e3, T_0_no, cmap='viridis')
    ax2.set_aspect('equal')
    c2 = plt.colorbar(mesh2, ax=ax2, pad=0.05, shrink=1, fraction=fraction_bar, aspect=np.shape(
        Z_ext_no)[0]/np.shape(Z_ext_no)[1]*((1-fraction_bar)/fraction_bar))
    c2.set_label('[s]')
    ax2.set_title(
        f"Dwell time =  {np.round((np.sum(T_0_no)), 2)} s")

    ax3 = fig.add_subplot(grid[plot_idx, 2])
    mesh3 = ax3.pcolormesh(
        X * 1e3, Y * 1e3, Z_residual_ca_0_no * 1e9, cmap='viridis')
    ax3.set_aspect('equal')
    c3 = plt.colorbar(mesh3, ax=ax3, pad=0.05, shrink=1, fraction=fraction_bar,
                      aspect=np.shape(Z)[0]/np.shape(Z)[1]*((1-fraction_bar)/fraction_bar))
    c3.set_label('[nm]')
    ax3.set_title(
        f"Residual \nPV =  {np.round((np.max(Z_residual_ca_0_no) - np.min(Z_residual_ca_0_no)) * 1e9, 2)} nm, RMS = {np.round(np.std(Z_residual_ca_0_no) * 1e9, 2)} nm")
    ax3.set_xlim(ax2.get_xlim())
    ax3.set_ylim(ax2.get_ylim())

    ax4 = fig.add_subplot(grid[plot_idx, 3])
    mesh4 = ax4.pcolormesh(X_ext_no * 1e3, Y_ext_no * 1e3,
                           Z_removal_dw_0_no * 1e9, cmap='viridis')
    ax4.set_aspect('equal')
    c4 = plt.colorbar(mesh4, ax=ax4, pad=0.05, shrink=1, fraction=fraction_bar, aspect=np.shape(
        Z_ext_no)[0]/np.shape(Z_ext_no)[1]*((1-fraction_bar)/fraction_bar))
    c4.set_label('[nm]')
    ax4.set_title(
        f"Removal \nPV =  {np.round((np.max(Z_removal_dw_0_no) - np.min(Z_removal_dw_0_no)) * 1e9, 2)} nm, RMS = {np.round(np.std(Z_removal_dw_0_no) * 1e9, 2)} nm")
...
if selection_savejson[0]:
    simulated_result = {map_name[0]: Z_0_no * 1e9, map_name[1]: T_0_no,
                        map_name[2]: Z_residual_ca_0_no * 1e9, map_name[3]: Z_removal_dw_0_no * 1e9}
